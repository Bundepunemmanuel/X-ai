    ...
    <div class="card" data-id="${draft.id}">
      ...
      <div class="card-body">
        ...
        <div class="card-actions">
          <button class="btn-approve" data-action="approve">Approve &amp; Open X</button>
...
function attachCardHandlers() {
  document.querySelectorAll(".card").forEach((card) => {
    ...
    card.querySelector('[data-action="approve"]').addEventListener("click", async () => {
      ...
      if (result.success && result.intent_url) {
        window.open(result.intent_url, "_blank");
        replaceWithDoneCard(card, "approved");
      } else {
        alert(result.error || "Could not build a link for this draft.");
      }
    });
...
function replaceWithDoneCard(card, status) {
  ...
  const label = status === "approved" ? `Opened X for ${handle} — tap Post there to send` : `Skipped — ${handle}`;
